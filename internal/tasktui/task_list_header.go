...
func (m Model) renderTaskListHeader(width int) string {
	...
	if width < 72 {
		return renderCompactTaskListHeader(width, meta, cwd, runtime)
	}
	return renderWideTaskListHeader(width, meta, cwd, runtime)
}

func renderWideTaskListHeader(width int, meta versionMeta, cwd, runtime string) string {
	hero := renderTaskListWordmark(width)
	version := centerHeaderLine(renderTaskListVersionMeta(meta), width)
	metaBlock := renderTaskListMetadataBlock(width, cwd, runtime, true)
...
func renderCompactTaskListHeader(width int, meta versionMeta, cwd, runtime string) string {
	top := joinHorizontal(
	...
	return lipgloss.JoinVertical(
		...
		top,
		hero,
		renderTaskListMetadataBlock(width, cwd, runtime, false),
	)
}
...
func renderTaskListMetadataBlock(width int, cwd, runtime string, centered bool) string {
	joined := renderTaskListMetaLine("cwd", cwd, false) +
		tuiTheme.Header.MetaLabel.Render("  •  ") +
		renderTaskListMetaLine("runtime", runtime, true)
	if ansi.StringWidth(ansi.Strip(joined)) <= width {
		if centered {
			return centerHeaderLine(joined, width)
		}
	...
	if centered {
		for i := range lines {
			lines[i] = centerHeaderLine(lines[i], width)
		}
